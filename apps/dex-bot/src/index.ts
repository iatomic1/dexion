...
import { type Context, type SessionStore, session, Telegraf } from "telegraf";
...
const store: SessionStore<SessionData> = Redis<SessionData>({
	url: "redis://127.0.0.1:6379",
});

const bot = new Telegraf<MyContext>(process.env.BOT_TOKEN || "");
bot.use(session({ store, defaultSession: () => ({ session_token: null }) }));
...
bot.start(async (ctx) => {
	...
	// Expected format: /start ott_verify_YOUR_GENERATED_OTT_TOKEN
	...
	if (messageText.startsWith(`/start ${startPayloadPrefix}`)) {
		...
		if (fullPayload.startsWith(startPayloadPrefix)) {
			...
			try {
				...
				if (error) {
					await ctx.reply("Authentication failed. Invalid or expired token.");
					return;
				}
				...
				if (data.session && data.user) {
					...
					await ctx.reply(
						`Welcome back, ${data.user.name}! You are now authenticated.`,
					);
				} else {
					await ctx.reply("Authentication failed. Invalid or expired token.");
				}
			...
			// Handle other /start payloads if you have them
...
bot.command("alert", async (ctx) => {
	...
	try {
		...
		ctx.reply(messages.ALERT_SET_SUCCESS(contractAddress, direction, price));
	} catch (error) {
		console.error(error);
		ctx.reply(messages.GENERIC_ERROR_MESSAGE);
	}
