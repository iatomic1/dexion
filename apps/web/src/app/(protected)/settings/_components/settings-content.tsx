...
import { WebhookConfig } from "@dexion/api-sdk/index.ts";
...
const mockWebhookConfig = {
	...
	webhook_url: "https://api.example.com/webhooks/price-alerts",
	bearer_token: "sk_live_abcdefghijklmnopqrstuvwxyz1234567890",
	enabled: true,
	status: "interrupted" as "streaming" | "interrupted",
	updated_at: new Date(),
			...
			<div className="mx-auto max-w-3xl px-6 py-12">
				...
				<div className="mb-12">
					...
					<div className="border border-border rounded-lg overflow-hidden bg-card">
						...
						{session ? (
							<DarkSettingsSection
								title="Link Bitflow Account"
								description={
									session?.user.externalAddress
										? "Linked: " +
											truncateString(session?.user.externalAddress, 10, 5)
										: "Connect your Stacks address to receive HODLMM price alerts on Dexion"
								}
								action={<LinkBitflow user={session.user} />}
								hasBorder
							/>
						) : (
							<div className="p-6 border-t border-border flex items-center justify-between">
				...
				<div className="mb-12">
					...
					<div className="border border-border rounded-lg overflow-hidden bg-card">
						<WebhookConfigSection initialConfig={webhookConfig || null} />
					</div>
				</div>
