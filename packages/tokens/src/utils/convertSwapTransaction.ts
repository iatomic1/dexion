...
export function transformFakSwapTx(
	input: {
		txId: string;
	...
	const burn_block_time = Math.floor(input.timestamp / 1000);
	const isBuy = input.type.toLowerCase() === "buy";

	const token_x_amount = isBuy
		? input.ustxAmount.toString()
		: input.tokensAmount.toString();

	const token_y_amount = isBuy
		? input.tokensAmount.toString()
		: input.ustxAmount.toString();

	const price_usd = input.pricePerToken;
	const volume_usd =
		price_usd * (isBuy ? input.tokensAmount : input.ustxAmount);
	...
	return {
		...
		token_x: isBuy
			...
			: {
					contract_id: input.tokenContract,
					decimals: options?.token_y_decimals ?? 18,
					image_url: options?.token_y_image ?? "",
					symbol: options?.token_y_symbol ?? "TOKEN",
				},
