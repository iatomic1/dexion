import { db, schema } from "@dexion/db";
			...
			account: {
				create: {
					after: async (account, ctx) => {
						...
						const userFromSession: WalletProvisionUser = {
							...sessionUser,
							subOrgCreated: sessionUser.subOrgCreated ?? false,
						};

						await onWalletProvision(userFromSession, false);
					},
				},
			},
		},
		...
		hooks: {
			after: createAuthMiddleware(async (ctx) => {
				if (
					...
					await onWalletProvision(userFromSession, true);
				}
			}),
		},
		session: {
			...
			twoFactor({
				otpOptions: {
					async sendOTP(data) {
						...
						try {
							await sendAuthEmail(user.email, "sign-in", data.otp);
						} catch (error) {
